...
def gen_gif(signal_history: list, action_history: list, ep_fn, num_iter: int, record_interval: int, duration: int, output_file: str, figsize=(15, 6), height_ratios=None):
  ...
  for i in range(num_images):
    ...
    epy1.append(ep_fn(signal_history[i], action_history[i]))
    epx2.append((i+1)*record_interval)
    epy2.append(ep_fn(signal_history[i], action_history[i]))
